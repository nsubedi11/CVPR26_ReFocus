...
class PointGenerator(nn.Module):
    ...
    def __init__(
        ...
        for i, pts in enumerate(self.buffer_points):
            print(f"Buffered points for level {i}: {pts.shape}")
    ...
    def forward(self, feats):
        ...
        for feat_len, buffer_pts in zip(feat_lens, self.buffer_points):
            assert feat_len <= buffer_pts.shape[0], f"Reached max buffer length for point generator: {feat_len} vs {buffer_pts.shape[0]}"
